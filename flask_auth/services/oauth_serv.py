...
class OauthServ:
    ...
    def check_source(cls) -> FlaskOAuth2App:
        '''The method checks the presence of provider registration in core.oauth,
        returns settings of this provider, in the form of FlaskOAuth2App
        application.'''
        oauth = cls.get_oauth()
        try:
    ...
    def get_user_info(cls, oauth_app: FlaskOAuth2App) -> SocialUserModel:
        '''The method accepts FlaskOAuth2App with received keys,
    ...
    def check_and_create_account(cls, oauth_app: FlaskOAuth2App) -> tuple[Response, HTTP]:
        '''The method checks if the user is in the database, and if not, creates it,
        returns a pair of tokens for the standard login.
        The entire move is based on the uniqueness of the email, as well as the calculation,
        that if there is an email in the social network it was validated and a user
        confirmed it.'''
